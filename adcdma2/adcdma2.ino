...
#define	OUTPUTS_PER_BLOCK	(4 * 4)	// 4 chunks, each chunk generates one output on each of 4 channels
#define	HEADER_SIZE		OUTPUTS_PER_BLOCK	// header is the same size as a block (in samples, not bytes)
...
#define ADCPIN1 A4		// A3 _must_ be the base pin.  Others are not consecutive.
#define ADCPIN2 A5		// A3 _must_ be the base pin.  Others are not consecutive.
#define ADCPIN3 A6		// A3 _must_ be the base pin.  Others are not consecutive.
#define NPINS 4
